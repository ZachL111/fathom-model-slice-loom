...
`fathom-model-slice-loom` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for slice workflows, centered on event replay, fixture event logs, and golden state snapshots.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 3.
